class PatientprovidersController < ApplicationController
  ...
  def create
    ...
    if @patientprovider.save #Try to write Patientprovider record to DB. Returns true if successful.
      ...
      if session[:patient_provider_redirect_flag] == 1
         session.delete(:patient_provider_redirect_flag)
  ...
  def delete
    ...
    if session[:patient_provider_unassign_redirect_flag] == 1
      session.delete(:patient_provider_unassign_redirect_flag)
      flash[:success] = 'Client Unassigned'
      redirect_to session[:patient_show_url]

    elsif session[:patient_provider_unassign_redirect_flag] == 2
      session.delete(:patient_provider_unassign_redirect_flag)
      flash[:success] = 'Provider Unassigned'
      redirect_to session[:provider_show_url]
    end
