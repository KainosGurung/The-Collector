class ProvidersController < ApplicationController
  before_filter :authenticate_admin_login, except: :admin_index
  before_filter :authenticate_super_admin_login, only: :admin_index
  ...
  def admin_index
    #Returns all active admin records sorted by name
    @provider_array = Provider.all.where(active: true, admin: true).order(provider_name: :asc)
  end
  ...
  def edit
    ...
    elsif  session[:self_super_admin] == 1
      session[:edit_self_provider_redirect_flag] = 3
    else
